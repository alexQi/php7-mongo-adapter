...
class Adapter {
    ...
    public function registerModel($model) {
        ...
        if (!isset($this->models[$this->dbName]))
            $this->models[$this->dbName] = [];
    ...
    public function isModelRegistered($collection) {
        ...
        if (!isset($this->models[$this->dbName]))
            return false;
        if (!isset($this->models[$this->dbName][$collection]))
            return false;
    ...
    public function getModel($collection) {
        if (!$this->isModelRegistered($collection))
            return null;
    ...
    public function find($collection, $filters = [], $options = []) {
        $this->checkDB();
        if (is_a($filters, '\MongoDriver\Filter'))
            $filters = [$filters];
        ...
        foreach ($filters as $filter) {
            ...
            reset($filter);
            $key                = key($filter);
            if (isset($filtersArray[$key])){
                $filtersArray[$key] = array_merge($filtersArray[$key],$filter[$key]);
            }else{
    ...
    public function insert($collection, $item) {
        return $this->bulkInsert($collection, [$item])[0];
    }
    ...
    /**
     * Delete an item based on search fields.
     *
     * @param string $collection
     * @param array| $params
     * @param array $update
     * @param integer $limit
     */
    public function delete($collection, $params, $limit = 1) {
        $this->checkDB();
        ...
        return $this->db->executeBulkWrite("$this->dbName.$collection", $bulk, $writeConcern);
    }
